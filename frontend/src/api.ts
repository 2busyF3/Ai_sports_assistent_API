export type TrainingGoal = "maintenance" | "strength" | "hypertrophy";
export type Profile = { height_cm: number; body_weight_kg: number; weight_updated_at: string; training_goal: TrainingGoal };
...
export type TrendPoint = { performed_at: string; best_set_score: number; total_volume_kg: number; strength_rating: number | null };
...
export const api = {
  dashboard: () => request<{ profile: Profile | null; latest_sleep_hours: number | null; training_goal: TrainingGoal | null; message: string; suggestions: { name: string; suggested_weight_kg: number; target_reps: number }[] }>("/api/dashboard"),
  profile: () => request<{ profile: Profile | null }>("/api/profile"),
  saveProfile: (height_cm: number, body_weight_kg: number, training_goal: TrainingGoal) => request<{ profile: Profile }>("/api/profile", { method: "PUT", body: JSON.stringify({ height_cm, body_weight_kg, training_goal }) }),
